...
public interface ProductRepository extends JpaRepository<Product,Long>{
	...
	// Xếp theo số lượng tồn giảm nhanh
	...
	// Xếp theo thơi gian tạo giảm dần
	...
	@Query(value="select * from `sgucoffee`.product where `sgucoffee`.product.product_name like %?1% and `sgucoffee`.product.category_id= ?2",nativeQuery = true)
	Page<Product> findByProduct_NameAndCategory_idContaining(String name, int category_id, Pageable pageable);
	
	@Query(value="select * from `sgucoffee`.product where `sgucoffee`.product.product_name like %?1%",nativeQuery = true)
